  ...
  const inner = (
    <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-card-border bg-card transition-all hover:-translate-y-0.5 hover:shadow-md">
      <div className="relative aspect-square overflow-hidden bg-muted">
        {/* width/height は CLS 防止のため明示。実画像比率は様々だが
            object-cover + aspect-square で常に正方形にトリミングされるので
            正方形ダミー寸法で OK。alt は brand/name が下に表示されるので
            decorative 扱いで空文字 (SR は周辺テキストで認識する)。 */}
